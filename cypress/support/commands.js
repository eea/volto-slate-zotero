...
Cypress.Commands.add(
  'openSlateContextSidebar',
  ({
    sidebarSelector,
    nodeSelector = 'span[id^="cite_ref"]',
    ...
    cy.get('body').then(($body) => {
      if ($body.find(sidebarSelector).length > 0) {
        return;
      }
    ...
    cy.get('body').then(($body) => {
      ...
      cy.get(`a[title^="${editTitle}"]`).first().click({ force: true });
    });

    cy.get(sidebarSelector).should('exist');
  },
);

Cypress.Commands.add('saveSidebarPopup', (sidebarSelector) => {
  cy.get(`${sidebarSelector} .form > .header.pulled button`)
    .first()
    .click({ force: true });
});

Cypress.Commands.add('cancelSidebarPopup', (sidebarSelector) => {
  cy.get(`${sidebarSelector} .form > .header.pulled button`)
    .eq(1)
    .click({ force: true });
});
